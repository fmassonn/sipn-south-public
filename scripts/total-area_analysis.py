...
for j_sub in range(n_sub):
  plt.text(29, 4.0 - j_sub * 0.3, info[j_sub][0] + " " + info[j_sub][3], color = col[j_sub])
  for j_for in range(n_for[j_sub]):
    ...
    plt.plot(np.arange(len(time)) + 1, series, color = col[j_sub], lw = (5.0 / n_for[j_sub]) ** 0.2 )
...
csv = pd.read_csv(filein, header = None)
series = csv.iloc[0][:]
plt.plot(np.arange(len(time)) + 1, series, color = [0.1, 0.1, 0.1], lw = 3.0, linestyle = "--")
plt.text(29, 4.0 - n_sub * 0.3, "OBS (NSIDC 0081)", color = "black")

filein = "../data/txt/OSI-401-b_000_total-area.txt"
csv = pd.read_csv(filein, header = None)
series = csv.iloc[0][:]
plt.plot(np.arange(len(time)) + 1, series, color = [0.1, 0.1, 0.1], lw = 3.0, linestyle = ":")
plt.text(29, 4.0 - n_sub * 0.3, "OBS (OSI-401-b)", color = "black")
